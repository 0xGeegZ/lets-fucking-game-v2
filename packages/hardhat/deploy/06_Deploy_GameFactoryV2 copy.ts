...
const func: DeployFunction = async function ({
  ...
  const { address: gameAddress } = await deployments.get('GameV2')
  ...
  if (gameFactoryNewlyDeployed)
    log(
      `✅ Contract GameFactoryV2 deployed at ${gameFactoryAddress} using ${gameFactoryGasUsed} gas`
    )

  log('Adding GameFactoryV2 to Keeper delegators')
  try {
  ...
  await delay(30 * 1000)
  try {
    log(`✅ Verifying contract GameFactoryV2`)
